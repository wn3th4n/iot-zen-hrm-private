    ...
    <uc-modal v-model:isOpen="isOpen" title="Dữ liệu chấm công" :width="600" @onSubmit="onSubmit()" @onClose="handleCancel()">
        <a-card size="small">
            <div class="d-flex flex-row gap-2 align-items-center">
                ...
                <div>
                    <span class="fw-bold text-primary">[{{ record?.MaNhanVien }}] {{ record?.HoVaTenNhanVien }}</span
                    ><br />
            ...
            <template #renderItem="{ item }">
                <a-list-item>
                    ...
                    <a-list-item-meta>
                        <template #title>
                            <b>
                                <uc-icon :name="item.HinhThuc_Icon" color="primary" /> - {{ item.Gio }} <span v-if="item.TenVanPhong"> - {{ item.TenVanPhong }}</span>
                            </b>
                        </template>
                        ...
                        <template #avatar>
                            <a-space :size="12">
                                <a-image :width="100" :src="apiReturnFile + item.HinhAnh" v-if="item.HinhAnh" />
                                <a-image
                                    v-else
