<template>
    <uc-form-modal v-model:isOpen="isOpen" ref="refFormModal" title="Chỉnh sửa nhóm chính sách" :formData="record" :rules="rules" :width="500" @onSubmit="onFinish()" :isSubmit="isLoading" @onClose="handleCancel()">
        <a-row :gutter="[10]">
            <a-col :span="24">
...
export default {
    ...
    methods: {
        ...
        async onFinish() {
            ...
            const res = await nhomChinhSachService.NhomChinhSach_LamThem_Update(params)
            if (res) {
                $this.handleCancel()
                $this.$emit('onFinish')
                $this.$message.success('Chỉnh sửa nhóm chính sách thành công!')
