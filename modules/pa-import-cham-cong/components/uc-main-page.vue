<template>
	<uc-layout>
		...
		<uc-handsontable :data="dsNhanVien" :nestedHeaders="templateHeader" :cell="cellStyle"
			height="calc(100vh - 57px)" :afterGetColHeader="setHeaderClass">
			...
			<hot-column v-for="(date, index) in dsNgay" :title="date" :data="'Ngay_' + date.toString().padStart('2', 0)"
				width="90" :readOnly="isViewedTemplate" style="background-color:red"
				:className="getColumnClassNgay(date, index, dsThu)"></hot-column>
		</uc-handsontable>
	</uc-layout>
</template>
<script>
	export default {
		...
		methods: {
			...
			async onSave() {
				...
				Confirm.confirm({
					content: 'Xác nhận lưu chấm công',
					onOk: async function () {
