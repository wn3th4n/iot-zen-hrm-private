<template>
    <uc-form-modal centered :isOpen="isOpen"  title="Thêm chính sách" :width="1400" ref="modalRef" @onClose="onClose()" @onSubmit="onSubmit()"
        :isSubmit="isLoading" :formData="form" :rules="rules">
        <a-row :gutter="[25]">
            <a-col :span="12">
                <a-row :gutter="[10]">
                    <a-col :span="24">
                    ...
                    <a-col :span="24">
                        <a-form-item  name="checkbox">
                            <a-checkbox>Cho phép nhân viên tạo đề xuất cho bản thân</a-checkbox>
                            <a-checkbox>Cho phép người quản lý tạo đề xuất cho nhân viên cấp dưới</a-checkbox>
                        </a-form-item>
                    </a-col>

                    <a-col :span="24">
                        <a-form-item label="File mẫu (.docx)" name="File_ChinhSach">
                            <uc-file-upload v-model:modelValue="form.File_ChinhSach" :multiple="false" title="Upload file"
...
<script>
    export default {
		...
		methods: {
            ...
            onSubmit() {
                const record = this.record.find((i) => {return i.LoaiChinhSach_Id === this.form.Active_Id})
