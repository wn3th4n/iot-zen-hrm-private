<template>
    <uc-form-modal v-model:isOpen="isOpen" ref="refFormModal" title="Thêm chính sách bậc thưởng" :formData="form" :rules="rules" :width="500" @onSubmit="onFinish" :isSubmit="isLoading" @onClose="handleCancel()">
        <a-row :gutter="[10]">
            <a-col :span="16">
                <a-form-item label="Tên bậc thưởng" name="TenBacThuong">
                    <a-input v-model:value="form.TenBacThuong" />
                </a-form-item>
            </a-col>
            <a-col :span="8">
...
export default {
    ...
    methods: {
        ...
        onFinish() {
            ...
            $this.isLoading = true
            ajaxCALL(
                '/work/HR_BacThuong_Insert',
                {
                    TenBacThuong: $this.form.TenBacThuong,
                ...
                (res) => {
                    $this.handleCancel()
                    $this.$emit('onFinish')
                    Vue.$toast.success('Tạo bậc thưởng thành công!', { position: 'top' })
