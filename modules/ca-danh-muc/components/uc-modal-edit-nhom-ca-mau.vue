<template>
    <uc-form-modal :isOpen="isOpen" title="Chỉnh sửa nhóm ca mẫu" :width="600" :formData="record" ref="modalRef" :rules="rules" :isLoading="isLoading" @onSubmit="onEdit()" @onClose="onCloseModal()">
        <a-row :gutter="[10]">
            <a-col :span="7">
                <a-form-item label="Mã nhóm ca mẫu" name="MaNhomCaMau">
                    <a-input v-model:value="record.MaNhomCaMau" />
                </a-form-item>
            </a-col>
            <a-col :span="17">
                <a-form-item label="Tên nhóm ca mẫu" name="TenNhomCaMau">
                    <a-input v-model:value="record.TenNhomCaMau" />
                </a-form-item>
            </a-col>
            <a-col :span="24">
...
export default {
    ...
    methods: {
        ...
        async onEdit() {
            const $this = this
            $this.isLoading = true
            const res = await nhomCaMauService.NhomCaMau_Update($this.record)
            if (res) {
                Vue.$toast.success('Chỉnh sửa nhóm ca mẫu thành công!', { position: 'top' })
