<template>
    <uc-form-modal :isOpen="isOpen" title="Chỉnh sửa loại ca" :width="600" :formData="formData" ref="modalRef" :rules="rules" :isLoading="isLoading" @onSubmit="onSubmit()" @onClose="handleCancel()">
        <a-row :gutter="[10]">
            <a-col :span="6">
        ...
        onSubmit() {
            ...
            ajaxCALL(
                ...
                (res) => {
                    ...
                    $this.handleCancel()
                    $this.$emit('onFinish')
                    Vue.$toast.success('Chỉnh sửa loại ca thành công!', { position: 'top' })
                },
                (err) => {},
            )
