	...
	<uc-modal v-model:isOpen="isOpen" title="Thông tin ca" :width="1200" @onClose="onCloseModal()">
		<a-card size="small">
			<a-dropdown :trigger="['click']" class="float-end">
				...
				<template #overlay>
					<a-menu>
						<a-menu-item @click="onRelease(record)" v-if="record.TrangThai_Mau !== 'blue'"><uc-icon
								name="calendar-check" />Xuất bản ca</a-menu-item>
						<a-divider class="my-1"></a-divider>
						<a-menu-item @click="onOpenModalHandOver(record)"><uc-icon name="account-switch" />Giao
							lại</a-menu-item>
						<a-menu-item @click="onOpenModalEdit()"><uc-icon name="table-edit" />Chỉnh sửa</a-menu-item>
						<a-divider class="my-1"></a-divider>
						<a-menu-item @click="onDelete(record)"><uc-icon name="trash-can-outline" color="red" />Xóa
							ca</a-menu-item>
					</a-menu>
				</template>
			</a-dropdown>
			<p class="mb-0 fw-bold text-primary">
				<uc-avatar :src="record?.AnhDaiDien_Url" :text="record?.HoVaTenNhanVien" size="18" /><span
					class="ms-2">[{{ record?.MaNhanVien }}] {{ record?.HoVaTenNhanVien }}</span>
			</p>
		...
		<a-row class="mt-3 mx-3">
			...
			<a-col :xs="24" :md="6"> <b>Khung giờ vào ca:</b> {{ record?.BatDauCa_Som }} → {{ record?.BatDauCa_Tre
				}}</a-col>
			<a-col :xs="24" :md="6"> <b>Khung giờ ra ca:</b> {{ record?.KetThucCa_Som }} → {{ record?.KetThucCa_Tre }}
			</a-col>
			...
			<a-col :xs="24" :md="6"> <b class="mb-1">Check-In thực tế:</b> {{ DSCheckInOut[0]?.Gio }}
				<uc-icon v-if="DSCheckInOut.length > 0" :name="DSCheckInOut[0]?.HinhThuc_Icon" />
			</a-col>
			<a-col :xs="24" :md="6"> <b class="mb-1">Check-Out thực tế:</b> {{ DSCheckInOut[DSCheckInOut.length -
				1]?.Gio }}
				<uc-icon v-if="DSCheckInOut.length > 0" :name="DSCheckInOut[DSCheckInOut.length - 1]?.HinhThuc_Icon" />
			</a-col>
		...
		<uc-modal-edit-ca v-model:isOpen="isOpenModalEdit" :record="RecordItem" :dsloaica="dsloaica"
			:dsvaitro="dsvaitro" :dsvanphong="dsvanphong" :dscamau="dscamau" @onFinish="onFinishEdit" />
