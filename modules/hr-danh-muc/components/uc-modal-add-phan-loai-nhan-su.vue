<template>
	<uc-form-modal v-model:isOpen="isOpen" ref="refFormModal" title="Thêm hình thức" :formData="form" :rules="rules"
		:width="600" @onSubmit="onFinish" :isSubmit="isLoading" @onClose="handleCancel()">
		<a-row :gutter="[10]">
			<a-col :span="24">
				<a-form-item label="Mã hình thức" name="MaHinhThuc">
					<a-input v-model:value="form.MaHinhThuc" autoFocus />
				</a-form-item>
			</a-col>
			<a-col :span="24">
				<a-form-item label="Tên hình thức" name="TenHinhThuc">
					<a-input v-model:value="form.TenHinhThuc" />
				</a-form-item>
			</a-col>
			<a-col :span="24">
...
export default {
	...
	methods: {
		...
		onFinish() {
			const $this = this
			$this.isLoading = true
			ajaxCALL("/work/HR_HinhThuc_Insert", {
				MaHinhThuc: $this.form.MaHinhThuc,
				TenHinhThuc: $this.form.TenHinhThuc,
				MoTa: $this.form.MoTa,
				Is_TamKhoa: $this.form.Is_TamKhoa
			}, (res) => {
				$this.handleCancel()
				$this.$emit('onFinish')
				Vue.$toast.success("Thêm hình thức thành công!", { position: "top" })
