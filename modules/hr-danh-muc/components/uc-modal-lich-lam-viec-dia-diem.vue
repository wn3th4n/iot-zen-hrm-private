<template>
    <uc-form-modal ref="refFormModal" v-model:isOpen="isOpen" :formData="form" :rules="rules" :width="1024" :title="'Thêm lịch làm việc cho ' + record?.TenVanPhong" @onSubmit="onFinish" :isSubmit="loading" @onClose="handleCancel()">
...
export default {
    ...
    computed: {},
    watch: {
        'record.VanPhong_Id': function (val) {
            if (val) {
    ...
    methods: {
        ...
        onFinish() {
            ...
            $this.loading = true
            ajaxCALL(
                '/work/HR_VanPhong_MauBangCong_Insert',
                {
