<template>
    <uc-form-modal :isOpen="isOpen" ref="refFormModal" title="Thêm khu vực địa lý" :formData="form" :rules="rules" :width="700" @onSubmit="onFinish" :isSubmit="isLoading" @onClose="handleCancel()">
        <a-row :gutter="[10]">
            <a-col :span="12">
                <a-form-item label="Mã khu vực địa lý" name="MaKhuVucDiaLy">
                    <a-input v-model:value="form.MaKhuVucDiaLy" />
                </a-form-item>
            </a-col>
            <a-col :span="12">
                <a-form-item label="Tên khu vực địa lý" name="TenKhuVucDiaLy">
                    <a-input v-model:value="form.TenKhuVucDiaLy" />
                </a-form-item>
            </a-col>
            <a-col :span="24">
...
export default {
    ...
    methods: {
        ...
        async onFinish() {
            ...
            const res = await khuVucDiaLyService.KhuVucDiaLy_Insert(params)
            if (res) {
                $this.handleCancel()
                $this.$emit('onFinish')
                $this.$message.success('Thêm khu vực địa lý thành công!')
