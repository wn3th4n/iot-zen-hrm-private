<template>
	<a-modal v-model="action.isShowModalAddCot" title="Thêm cấu hình cột" :width="1500">
					...
					<a-row :gutter="ROW_GUTTER">
						<a-col :span="6">
							<a-form-item label="ID hệ thống" prop="MaHeThong">
								<a-input v-model="form.MaHeThong" autoFocus/>
							</a-form-item>
						</a-col>
						<a-col :span="6">
						...
						<a-col :span="6">
							<a-form-item label="Công thức theo" prop="Is_TheoNgay">
								<a-select v-model="form.Is_TheoNgay">
...
<script>
	export default {
	    props: ["action", "onCreateFinish", "maubangluong", "DSBienHeThong"],
	    data() {
	        return {
	            loading: false,
	            form: {
	            ...
	            rules: {
	                ...
	                Is_TheoNgay: [
	                    { required: true, message: "Vui lòng chọn loại công thức", trigger: "change" }
	                ],
	    ...
	    methods: {
	        ...
	        onSubmit() {
	            this.$refs.ruleForm.validate(valid => {
	                if (valid) {
	                    ajaxCALL("/work/CA_MauBangCong_ThuocTinh_Insert", {
	                        MauBangCong_Id: this.maubangluong.MauBangCong_Id,
