<template>
    <uc-form-modal v-model:isOpen="isOpen" title="Chỉnh sửa cột" :width="1400" :formData="formData" ref="modalRef" :rules="rules" :isLoading="isLoading" @onSubmit="onSubmit()" @onClose="handleCancel()">
        <a-row :gutter="[10]">
            <a-col :span="8">
...
export default {
    emits: ['onFinish'],
    props: ['isOpen', 'cauhinhcot', 'DSBienHeThong'],
    data() {
        return {
            isLoading: false,
            rules: {
    ...
    methods: {
        ...
        onSubmit() {
            ...
            $this.isLoading = true
            ajaxCALL(
                '/work/PA_MauBangLuong_ThuocTinh_Update',
                {
                    ...
                    MauBangLuong_ThuocTinh_Id: $this.formData.MauBangLuong_ThuocTinh_Id,
