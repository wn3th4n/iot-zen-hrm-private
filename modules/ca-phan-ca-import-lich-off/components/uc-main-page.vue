	...
	<uc-layout>
			...
			<template #extra>
				<a-space size="small">
					...
					<a-button v-if="!isViewedTemplate" type="primary" @click="onSave()"><uc-icon
							name="content-save-outline" />Lưu lịch nghỉ</a-button>
					<a-button type="primary" danger @click="onDelete()"><uc-icon name="delete-outline" />Xóa lịch
						nghỉ</a-button>
				</a-space>
			</template>
		</a-card>
		<uc-handsontable :data="dsNhanVien" :nestedHeaders="templateHeader" height="calc(100vh - 57px)">
			...
			<hot-column v-for="(date, index) in dsNgay" :title="date" :data="'Ngay_' + date.toString().padStart('2', 0)"
				width="30" :readOnly="isViewedTemplate"></hot-column>
		</uc-handsontable>
	</uc-layout>
</template>
<script>
	export default {
	    ...
	    methods: {
	        ...
	        async onSave() {
	            ...
	            Confirm.confirm({
	                content: 'Xác nhận lưu lịch nghỉ',
	                onOk: async function () {
	                    const params = {
	                        LichLamViec_Id: $this.LichLamViec_Id,
	                        MauBangCong_Id: $this.MauBangCong_Id,
